...
- **记录库服务端过滤：** `GET /api/library` 支持 `category`、`year`、`status`、`query`、`source` 和 `review`，列表、后续分页与 totals 使用同一条件。`category=media` 等于 `movie + tv_show`；`query` 最长 200 字符；`review` 为 `reviewed|unreviewed`。
...
- `GET /api/library` 支持游标分页：`?cursor=...&limit=50&category=media&year=2026&status=DONE&query=标题&source=douban&review=reviewed`
