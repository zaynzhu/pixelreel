...
import com.pixelreel.config.XboxProperties;
...
public class XboxGameSearchProvider implements GameSearchProvider {
  private final XboxProperties xboxProperties;
  ...
  public ProviderSearchResult<ExternalGameSearchResult> search(String query, int page) {
    ...
    if (!xboxProperties.enabled()) {
      result.setEnabled(false);
      result.setMessage("Xbox 未启用，需要确定可用的官方或第三方搜索 API");
      result.setPage(page);
    ...
    if (pspricesProperties.enabled()) {
      ...
      result.setEnabled(false);
      result.setMessage("Xbox 搜索 API 尚未配置");
    }
    result.setPage(page);
    result.setTotalPages(0);
    result.setTotalResults(0);
