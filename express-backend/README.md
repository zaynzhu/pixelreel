...
| POST | `/api/tools/restore` | 上传同一快照和一次性确认令牌，只创建快照独有数据 |
...
14. **导入参数校验**：导入和回填 `limit` 默认 50、范围 1-100；状态、豆瓣模式和平台标识参数无效时返回 400，不启动任务。
15. **记录编辑校验**：记录 ID 必须是安全正整数；Library PATCH 只允许状态、1-5 分评分和不超过 1000 字符的短评。
16. **错误脱敏**：4xx 返回可操作提示，5xx 统一返回“内部服务器错误”；内部异常与堆栈仅写服务端日志，响应不包含框架指纹。
17. **活动日志参数**：分页数量、游标、实体 ID 和日期范围均在查询前校验，无效参数返回 400。
18. **增量恢复**：恢复确认令牌绑定快照和现库指纹、十分钟过期且只能使用一次；写入前先
    生成 `0600` 安全备份，并在可串行化事务中只创建快照独有数据，不覆盖或删除现库记录。
