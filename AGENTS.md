...
- **主机游戏完整性：** PSNProfiles 按 `?ajax=1&page=N` 读取全部游戏页，直到页面声明 `nextPage = 0`，最多 100 页；`psnId` 只保存 `/trophies/{数字ID}-{slug}` 中稳定的数字 ID，不保存可变标题 slug；HTTP 200 验证页和 Cloudflare 403 验证响应都必须提示更新 Cookie。Xbox/PSN 原始记录缺封面时通过 RAWG 按标题回退查询，仍遵守同服务两秒限流。
