...
- **仪表盘语义：** 首页统计按钮只重新读取 `/api/profile/summary`，必须标为“刷新统计”，不能伪装成同步操作；共享 `profileStore` 的并发读取必须采用最新请求获胜。初次读取失败必须显示原因和重试，不能用全零指标伪装成空库；已有摘要刷新失败时保留旧数据并明确报错。游戏遥测按 `GamePlatformEntry` 汇总游玩时长、平台档案数和可靠的已解锁成就/奖杯数量，同时按平台展示游玩时长覆盖、成就数据覆盖、仅有已解锁数的档案数量和最近同步时间；无平台档案的遗留记录才回退旧游戏字段，不能同时累计两套字段。真实同步统一进入 `/sync`。最新入库记录直接链接统一详情页。
