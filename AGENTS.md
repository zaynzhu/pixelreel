...
- **导入审核：** 历史记录和手动新增默认 `ACCEPTED`；豆瓣、Trakt、Steam 等外部导入的新记录显式写入 `PENDING`。`/sync/review` 可按豆瓣、Trakt、Steam、Xbox、PSN 来源筛选，并通过首屏列表请求的 `includeSourceCounts=true` 显示当前标签下的全局来源数量；切换来源后计数不能退化成当前来源的局部数量，普通资料库请求不计算该分组。游戏记录直接展示可信的成就/奖杯进度，并通过只读 `GET /api/data-health/duplicates/game-hints?ids=` 提示当前已加载记录涉及的未裁决重复候选；接受单条或批量记录时，只要所选记录包含已读取到的重复候选，就必须明确提示涉及数量并由用户二次确认。提示链接必须携带候选组指纹以及经过白名单校验的站内返回上下文，数据健康页自动追加读取分页直到定位并高亮具体候选，返回时恢复原标签、来源和记录位置，候选或原记录已变化、合并、裁决时保留原筛选并明确提示，读取失败不能阻断审核队列。记录可批量改为 `ACCEPTED` 或 `IGNORED`；忽略仅修改 `importReviewState`，不能删除记录或改写豆瓣原始字段。标签、来源筛选、分页和决定后的刷新必须同时校验当前视图与最新请求，不能让旧视图响应覆盖当前列表；首屏读取失败必须显示具体原因和重试，已有队列的刷新或分页失败必须保留已读记录并可重试原请求。
