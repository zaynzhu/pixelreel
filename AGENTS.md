...
- **重复候选：** `/data-health/duplicates` 只读分组外部 ID 相同的记录；无共同外部 ID 时，影视仅按规范化标题+年份，游戏按规范化标题+同平台或跨平台匹配。标题规范化必须保留连接符号的语义，不能因删除 `&` 等符号把罗马数字合集与续作折叠成相同标题。候选可逐条纠正辅助元数据，或整组标记为“确认不同”并恢复；裁决指纹随成员和共享标识变化而失效。游戏合并前必须通过只读预览重新读取当前候选，展示保留记录、移除记录数、平台档案迁移数量、合并后的个人状态/评分/短评以及全部阻断原因；存在冲突时不能提供最终合并操作。系统不能按标题自动合并或删除；用户确认的游戏合并必须在同一事务内迁移平台档案、保存完整恢复快照并写入不可伪装的 `MERGE` 活动，且可从活动日志安全撤销；撤销前若保留记录或平台档案已变化则必须拒绝。豆瓣来源身份和原始字段必须保留。
