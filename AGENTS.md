...
| `/library` 记录库；`/library/:category/:id` 统一详情页 | `GET /api/library?cursor=&limit=50&includeTotals=&includeSourceCounts=&category=&year=&status=&query=&source=&review=&importReview=&sort=`, `GET /api/library/:category/:id`, `PATCH /api/library/:cat/:id` |
...
| `/data-health` | `GET /api/data-health/summary`, `GET /api/data-health/issues`, `GET /api/data-health/duplicates`, `GET /api/data-health/duplicates/game-hints`, `POST/DELETE /api/data-health/duplicates/review`, `POST /api/data-health/duplicates/merge-preview`, `POST /api/data-health/duplicates/merge`, `POST /api/data-health/repair`（数据完整性审计、重复候选提示与裁决、游戏合并预览和定向修复） |
...
- **Settings 页面：** 敏感配置只返回 `configured` 状态，不回传现有明文；密码框留空表示保留原值，输入新值才会覆盖。保存期间锁定整个表单且只允许一个在途请求，页面卸载后旧响应不能更新状态；敏感值保存成功后必须立即从前端状态和输入框清除，只保留已配置标记。写入时校验已知字段、布尔/数字类型及危险字符，并通过同目录临时文件原子替换 `.env`。OpenXBL、Microsoft Xbox 与 PSNProfiles 字段保存后同时更新运行时配置且不要求重启；混合请求中的其他字段仍提示重启，已有重启提示不能被后续即时生效的保存清除。
- **Settings 读取：** 配置读取必须采用最新请求获胜，页面卸载时使在途请求失效；已有配置的重新读取失败时保留当前表单并持续显示错误与重试入口，不能只弹 Toast 后留下空白配置区；读取期间临时锁定表单，存在未保存改动时不能直接重试覆盖。
- **国际化：** Zustand `i18nStore`，提供 `t()` 函数，EN/ZH 字典，持久化到 localStorage。所有新组件必须 i18n；页面根元素的 `lang` 必须随当前语言同步为 `en` 或 `zh-CN`，图标和分页圆点等无文字控件必须提供对应语言的可访问名称。
...
- **数据健康：** `/data-health` 只读审计显示字段缺口，不自动修改记录；电影/剧集检查封面、简介、日期和外部 ID，游戏只检查封面和外部 ID，问题列表按 BigInt ID 游标分页。字段问题与重复候选的首屏、分页和操作后刷新必须绑定当前类别/筛选与最新请求，不能跨视图混入旧结果；摘要、字段队列和重复候选错误必须独立显示并可重试，分页失败必须保留已有结果和原游标并只允许显式重试原请求，打开候选详情失败不能污染列表读取错误；读取失败不能显示“没有问题”的健康空状态。
- **统一记录详情：** `/library/:category/:id` 展示个人状态、评分、短评、来源身份与原始字段、游戏指标和记录级操作历史，并可保存个人记录、重新匹配元数据或进入数据健康页。游戏平台档案必须分别展示平台身份、首次导入、最近同步、游玩时长和可信的成就或奖杯进度；顶部覆盖概览只统计含时长或成就/奖杯数据的平台档案数量及最近同步时间，不得跨平台合计时长、成就或奖杯。非空的零分钟表示已知为零，覆盖数应计入且卡片显示零时长，只有 `null` 才显示暂无数据；进行中状态仍只由大于零的时长推导。PSN 使用“奖杯”、其他平台使用“成就”，只有总数为正且已解锁数不超过总数时才显示完成率，未知总数不得伪造比例。记录库、时间线、活动日志和 Showcase 均提供详情入口；路由变化或页面卸载时必须使旧详情读取失效，旧成功或旧错误不能覆盖当前记录；有效地址读取失败必须显示具体原因和原地重试，无效地址只提供返回入口。
- **记录库读取：** 记录库首屏、筛选与分页必须绑定当前列表请求；切换筛选时立即清空旧筛选记录和分页状态，同条件刷新失败时保留已有记录与游标；旧分页的成功或失败不能追加或污染新列表，首屏、刷新和分页失败都必须显示原因并可重试原请求。游戏卡片的遥测摘要必须优先逐条读取 `GamePlatformEntry`，分别标明 PSN 奖杯和其他平台成就，并使用最新平台档案同步时间；总数未知时保留可信的已解锁数量，不能因缺少总数而隐藏，只有无平台档案的遗留记录才回退游戏旧字段。
- **仪表盘语义：** 首页统计按钮只重新读取 `/api/profile/summary`，必须标为“刷新统计”，不能伪装成同步操作；共享 `profileStore` 的并发读取必须采用最新请求获胜。初次读取失败必须显示原因和重试，不能用全零指标伪装成空库；已有摘要刷新失败时保留旧数据并明确报错。`overview.pendingImports` 必须统计 Movie、TvShow、Game 中 `importReviewState=PENDING` 的总数，首页以行动入口显示并链接 `/sync/review?tab=pending&source=all`，不能把已忽略记录计入。首页同步入口必须复用同步中心的浏览器公开账号覆盖和来源可用性计算，以默认 Microsoft Xbox 来源显示待处理数量；来源状态读取失败时保留普通入口，不能显示虚假的零待处理。游戏遥测按 `GamePlatformEntry` 汇总游玩时长、平台档案数和可靠的已解锁成就/奖杯数量，同时按平台汇总游玩分钟与已解锁数量，并展示游玩时长覆盖、成就数据覆盖、仅有已解锁数的档案数量和最近同步时间；PSN 的已解锁总量使用“奖杯”，其他平台使用“成就”。无平台档案的遗留记录才回退旧游戏字段，不能同时累计两套字段。真实同步统一进入 `/sync`。最新入库记录直接链接统一详情页。
...
- **展示布局：** Showcase 的统计横条、海报墙、时间线和随机推荐在窄屏必须按阅读顺序纵向排列，统计数字和分类卡片不得撑宽页面；390px 视口不得出现页面级横向滚动，桌面端仍保持统计横条与下方 60/40 双栏布局。
- **海报轮换：** Showcase 自动海报墙的定时请求必须采用最新批次获胜；慢旧批次的成功或失败不能覆盖已经显示的新批次。当前批次失败时保留最后成功海报，明确显示轮换错误并提供原地重试，不能静默回退到初始列表。
- **数据健康修复：** `POST /api/data-health/repair` 每次最多处理 50 条并创建唯一后台任务，只填充所选空字段及对应空 TMDB 原始字段；电影/剧集使用 TMDB，游戏仅支持 RAWG 封面，游戏外部 ID 必须人工核对，不能按标题自动绑定。任务在外部请求期间被取消后必须在下一次请求和数据库写入前再次检查取消信号，不能继续落库。
- **重复候选：** `/data-health/duplicates` 只读分组外部 ID 相同的记录；无共同外部 ID 时，影视仅按规范化标题+年份，游戏按规范化标题+同平台或跨平台匹配。标题规范化必须保留连接符号的语义，不能因删除 `&` 等符号把罗马数字合集与续作折叠成相同标题。候选必须显示每条记录的导入审核状态，方便区分待审核导入与正式记录；`scope=all|pending` 必须先在服务端完整过滤再分页，并返回当前裁决标签下的全部与待审核候选计数，不能只过滤前端已加载页面。逐组核对模式一次只能展示一组，必须将本次已完成数量与当前剩余队列位置分开显示；处理成功后继续下一组，当前范围清空后提供返回导入审核入口。支持前后切换，但不得因此批量确认、批量合并或跳过每组的当前预览。游戏候选组恰好只有一条 `ACCEPTED` 记录时可将其标记为建议主记录；没有或存在多条正式记录时不得按平台、标题或 ID 猜测推荐，推荐也不得自动选择或合并。候选可逐条纠正辅助元数据，或整组标记为“确认不同”并恢复；裁决指纹随成员和共享标识变化而失效。游戏合并前必须通过只读预览重新读取当前候选，逐条展示保留与并入记录的标题、ID、全部平台和导入审核状态；平台档案必须列出平台、公开外部 ID、原记录 ID、游玩或成就指标以及保留或迁移去向。预览同时展示移除记录数、平台档案迁移数量、合并后的个人状态/评分/短评/导入审核状态以及全部阻断原因；只要任一候选已正式入库，合并结果必须保持正式入库，存在冲突时不能提供最终合并操作。系统不能按标题自动合并或删除；用户确认的游戏合并必须在同一事务内迁移平台档案、保存完整恢复快照并写入不可伪装的 `MERGE` 活动，且可从活动日志安全撤销；撤销前若保留记录或平台档案已变化则必须拒绝。豆瓣来源身份和原始字段必须保留。
...
- **资料库快照：** `GET /api/tools/export-library` 在只读事务中按 ID 导出全部 Movie/TvShow/Game 字段，并按平台和公开外部 ID 稳定排序导出全部 `GamePlatformEntry`；包含豆瓣原始字段和导入审核状态。快照格式为 `pixelreel-library-export` v2，清单包含三类主记录、平台档案计数以及对规范化 `records` JSON 计算的 SHA-256；下载响应同步返回版本、计数和校验值头，跨域时必须显式暴露这些响应头。前端必须同时校验正文清单、响应头并重新计算记录区 SHA-256，全部一致后才能保存文件。不包含环境变量、Settings 或任何凭据，也不提供覆盖/恢复写入入口。
...
- **主机平台导入：** Xbox 支持 Microsoft 官方 OAuth 账号登录与 OpenXBL 两种来源。默认 Microsoft 登录复用 OpenXbox 公开桌面客户端 ID，不需要用户注册 Azure，使用本机 `127.0.0.1:8080` 临时桥接已登记回调；自有 Azure 应用只作为高级备用。Microsoft 链路为 OAuth → Xbox User Token → XSTS → title history，refresh token 连同 `community|custom` 来源标记仅以 `0600` 权限保存在本机 `data/xbox-microsoft-auth.json`，不得通过 API 回传；OpenXBL 链路为 `/search/{gamertag}` → XUID → `/player/titleHistory/{xuid}`。新记录统一进入导入审核队列，不覆盖个人状态、评分或短评。
- **同步中心：** `/sync` 集中展示豆瓣、Trakt、Steam、Xbox、PSN 的配置可用性和同步入口；`GET /api/import/sources/status` 只返回缺失原因，不返回凭据。顶部“需处理”数量必须基于当前选择的 Xbox Provider 和浏览器临时账号覆盖后的实际可用性，不能把已填写临时账号的来源误报为未配置。Xbox 与 PSN 未保存默认账号时必须报告 `missing_account`，任务接口省略账号参数时使用默认值；如果唯一缺口是默认账号，填写当前同步的临时账号后应立即开放同步，来源未启用或 Xbox 缺少 API Key 时仍必须禁用。Xbox Gamertag 与 PSN Online ID 只在用户明确勾选后保存到当前浏览器，取消勾选必须立即移除本地记录；密码、Cookie 和授权令牌不得写入该存储。Xbox/PSN 连接验证必须只读：Xbox 只确认精确账号及 title history 可访问，PSN 只读取档案第一页；不得创建任务、查询或写入资料库，修改临时账号、重新读取来源状态或卸载页面时必须使旧验证结果失效。`GET /api/import/sources/history` 从 `data/sync-history.json` 返回每个正式来源最近一次终态摘要，不含凭据；当前任务优先展示，历史摘要作为次级信息并随任务状态变化刷新，历史读取失败不能阻断来源状态与同步入口。同步历史同时显示精确时间和本地化相对时间，仅当完成时间距当前超过 30 天时提示“建议重新同步”，不能把正常的新鲜历史误报为过期。零结果且只有错误的同步必须记为失败并同时持久化完整结果摘要，来源正常返回空列表时仍记为完成，存在有效条目但伴随局部错误时保留完成状态和错误摘要；同步结果默认紧凑显示首条错误，存在多条时必须允许按需展开完整错误数组。来源状态和历史记录读取必须采用最新请求获胜，依赖变化或页面卸载时使在途请求失效；已有来源状态的重试期间和失败后必须保留来源卡片，并明确提示状态可能过期，重新确认成功前禁止启动新同步。全局 `taskStore` 的定时与手动轮询也必须采用最新请求获胜，同一时间最多一个在途任务读取，停止轮询时使旧响应失效。Steam 使用 `/api/import/steam/owned/task`，Trakt 使用 `/api/trakt/import/{movies|shows}/task`，Xbox 使用 `/api/import/xbox/owned/task`，PSN 使用 `/api/import/psn/owned/task`，配置缺失时跳转到 Settings 对应分类。
...
- **时间线：** `/timeline` 的主列表、分页与年份列表必须绑定当前分类/年份和最新请求；切换分类或年份时立即清空旧筛选记录和分页状态，同条件刷新失败时保留已有记录与游标；旧分页失败不能污染新筛选，首屏、刷新和分页失败必须显示错误并重试原请求，不能同时伪装为空状态；时间线与 Showcase 共用的按需详情弹窗读取失败时必须显示具体原因和原地重试，并在窄屏限制于视口内由弹窗本身滚动，不能把标题或关闭入口顶出屏幕。
...
- **命令抽屉：** 豆瓣、Trakt、Steam 快捷操作必须复用与同步中心相同的持久化任务端点，并由全局 `taskStore` 显示运行状态和冲突；不能调用旧同步接口或在抽屉内自行轮询。长时间数据修复统一跳转 `/data-health`，不在抽屉内直接执行。命令抽屉和全局任务面板关闭时不得把隐藏内容计入页面滚动宽度，打开时在窄屏内完整可见且仍支持 Escape 关闭和焦点恢复。
- **导入审核：** 历史记录和手动新增默认 `ACCEPTED`；豆瓣、Trakt、Steam 等外部导入的新记录显式写入 `PENDING`。`/sync/review` 可按豆瓣、Trakt、Steam、Xbox、PSN 来源筛选，并通过首屏列表请求的 `includeSourceCounts=true` 显示当前标签下的全局来源数量；切换来源后计数不能退化成当前来源的局部数量，普通资料库请求不计算该分组。游戏记录直接展示可信的成就或奖杯进度，PSN 标为“奖杯”，其他平台标为“成就”；并通过只读 `GET /api/data-health/duplicates/game-hints?ids=` 提示当前已加载记录涉及的未裁决重复候选；该响应同时返回全部未裁决且涉及 `PENDING` 导入的重复组总数，审核页提供直接打开对应筛选的集中处理入口。已加载记录必须区分已完成重复检查且无候选、存在候选和尚未检查三种状态；安全批量选择只能包含已检查且无候选的记录，重复检查失败或尚未完成时禁止把对应游戏改为 `ACCEPTED`，但仍允许查看、修正或忽略。接受单条或批量记录时，只要所选记录包含已读取到的重复候选，就必须明确提示涉及数量并由用户二次确认。提示链接必须携带候选组指纹、`scope=pending` 筛选以及经过白名单校验的站内返回上下文，数据健康页自动追加读取分页直到定位并高亮具体候选，返回时恢复原标签、来源和记录位置，候选或原记录已变化、合并、裁决时保留原筛选并明确提示，读取失败不能阻断审核队列。记录可批量改为 `ACCEPTED` 或 `IGNORED`；忽略仅修改 `importReviewState`，不能删除记录或改写豆瓣原始字段。标签、来源筛选、分页和决定后的刷新必须同时校验当前视图与最新请求，不能让旧视图响应覆盖当前列表；首屏读取失败必须显示具体原因和重试，已有队列的刷新或分页失败必须保留已读记录并可重试原请求。
- **主机游戏完整性：** OpenXBL 与 PSNProfiles 基础地址只允许不含查询参数或片段的绝对 HTTP(S) URL；OpenXBL 地址必须去除尾部斜杠后再拼接资源路径。现代 Gamertag 允许 `#数字后缀`，调用模糊搜索接口后必须同时读取 `gamertag`、`uniqueModernGamertag` 和 `modernGamertag` + `modernGamertagSuffix`，再按完整 Gamertag 忽略大小写精确匹配，不能直接采用第一条结果；匹配记录中的 XUID 仅接受 1-20 位正十进制数字。Xbox 同批次响应按 `titleId` 去重，只保留首次出现的游戏；合法空 `titles` 数组可正常完成，缺失或类型错误的游戏列表必须失败，不能伪装成空账号。PSNProfiles 分页请求必须复用站点的 `ajax=1&completion=all&order=last-played&pf=all&page=N` 参数和 XHR 请求头，以页面最后一个 `nextPage` 标记判断游戏列表是否结束，最多 100 页；只解析同时包含 `picture.game` 和 `/trophies/` 标题链接的游戏行，不能把最近奖杯动态误当成游戏；`psnId` 只保存 `/trophies/{数字ID}-{slug}` 中稳定的数字 ID，不保存可变标题 slug；相对 PSN 封面路径必须按站点地址解析，Xbox、PSN 与 RAWG 回退封面最终都只允许字段长度内的绝对 HTTP(S) URL；游玩时长与成就指标只接受 Prisma `Int` 范围内的非负整数。Xbox 成就总数为 `0` 或小于已解锁数时必须按未知总数处理，保留可信的已解锁数但不能展示无效比例，也不能破坏性清理数据库原始快照；其他异常指标忽略但不阻断游戏导入。OpenXBL 的认证、权限、资源不存在和限流响应必须转换为可操作提示，PSNProfiles 的档案不存在、限流、HTTP 200 验证页和 Cloudflare 403 验证响应也必须明确提示。OpenXBL、PSNProfiles 和 RAWG 回退请求均限制为 30 秒并支持任务取消；同服务请求仍遵守两秒限流。
...
- **年份筛选：** AnalyticsService 中「已完成」对豆瓣影视优先使用严格合法的 `doubanDate`，缺失或非法时回退 `updatedAt`；其他来源使用 `updatedAt`。豆瓣日期按原始年月日归属，时间戳按服务所在时区的本地日历归属，年份区间、可用年份和月度统计不得混用 UTC 与本地月份。当前年度的完成数和变化率只统计截至当天，并与去年同期比较；历史年份使用完整年度并与上年全年比较，前端必须明确显示比较口径。评分、短评、Top 榜、来源分布和跨平台评分使用 `createdAt`。本年入库游戏的来源图必须按每条 `GamePlatformEntry` 统计游戏平台档案，一条跨平台游戏可以分别贡献多个平台档案；只有无有效平台档案的遗留游戏才回退旧来源判定，前端不能把该图标成游戏记录数。
...
- **分析布局：** `/analytics` 的标题与年份控制、总览卡片、图表组合和来源圆环必须在窄屏改为纵向或分行布局；390px 视口不得出现页面级横向滚动，桌面端仍保持五列总览和双图并排。
...
- 前端 `AppShell` 生命周期内持续轮询任务列表，顶部 TASKS 数量不依赖任务面板是否打开；轮询失败时保留最后一次成功状态并在顶部入口、同步中心和任务面板明确警示，成功后自动清除。同步中心和全局任务面板的取消操作必须捕获失败并显示 Toast，不能产生未处理 Promise；全局任务面板对带局部错误的完成结果默认显示首条原因，并允许按需展开完整错误数组。
