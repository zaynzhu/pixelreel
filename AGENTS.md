...
- **主机平台导入：** Xbox 通过 OpenXBL 的 Gamertag → XUID → 游戏历史链路导入，要求启用 OpenXBL 并配置 API Key；PSN 逐页读取公开 PSNProfiles 档案，要求启用 PSNProfiles，遇 Cloudflare 验证页时更新 Cookie。同步中心要求用户输入平台账号；新记录统一进入导入审核队列，已有记录只刷新平台、空封面、游玩时长和成就指标，不覆盖个人状态、评分或短评。
