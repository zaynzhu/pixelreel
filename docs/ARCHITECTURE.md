...
| `ActivityLog` | CRUD、任务、合并、增量恢复和撤销事件 |
...
- 忽略导入只修改审核状态，不删除记录。
- `POST /api/library/import-review` 在可串行化事务中先复核整批目标，再更新并核对实际数量；
  任一目标已变化时返回 `409` 并回滚整批，不产生部分审核结果。
...
v2 JSON → 格式/版本/计数/SHA-256 校验 → 与当前资料库只读比较
  ...
  → 无冲突且有快照独有数据 → 一次性确认令牌
  → 二次确认 → 现库安全备份 → 可串行化事务重验指纹
  → 只创建快照独有数据 → RESTORE 活动日志
```
...
- `POST /api/tools/restore-preview` 只接受最大 50 MiB 的 v2 JSON，校验后按记录 ID、稳定来源
  身份和平台公开身份比较现库。预览本身只读，不创建活动日志。
- 无身份冲突且存在快照独有数据时，预览返回绑定快照 SHA-256 与现库指纹的十分钟一次性确认
  令牌；重启后端、令牌过期、快照变化或现库变化都要求重新预览。
- `POST /api/tools/restore` 必须上传同一快照并提供确认令牌。服务先创建权限为 `0600` 的现库
  安全备份，再在可串行化事务中重新读取和校验现库指纹。
- 增量恢复只创建快照独有的主记录和平台档案；内容不同的记录不覆盖，现库独有记录不删除，
  身份冲突阻断整次操作。带豆瓣 ID 的新建记录恢复后继续受禁止删除规则保护。
- 恢复完成后写入不可撤销的 `RESTORE / LIBRARY` 活动。不可撤销是为了不通过恢复功能绕过
  豆瓣删除红线；安全备份用于审计与人工核对，不代表自动回滚。
