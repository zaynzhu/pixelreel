...
```text
当前资料库 → v2 JSON + 分类计数 + 记录区 SHA-256
v2 JSON → 格式/版本/计数/SHA-256 校验 → 与当前资料库只读比较
  → 快照独有 / 内容不同 / 完全相同 / 身份冲突 / 现库独有
```

- `GET /api/tools/export-library` 在同一事务读取电影、剧集、游戏和稳定排序的平台档案。
- `POST /api/tools/restore-preview` 只接受最大 50 MiB 的 v2 JSON，校验后按记录 ID、稳定来源
  身份和平台公开身份比较现库。
- 预览只读取数据库，不创建活动日志，不提供应用按钮，也不执行创建、更新、合并或删除。
- “现库独有”是差异分类，不表示未来恢复会删除现有记录；实际恢复仍需独立设计和确认。
