...
- **导入审核队列** -- 新同步记录先进入待审核区，可按豆瓣、Trakt、Steam、Xbox、PSN 等稳定来源身份筛选并直接查看各来源数量和游戏成就进度；未裁决的跨平台重复候选会在接受前提示，并深链接定位到数据健康页的具体候选组，数据健康页可完整筛选所有涉及待审核导入的候选，候选与合并预览会标明审核身份，并逐条列出保留、并入记录的平台与 ID，返回时恢复原标签、来源和记录位置，忽略不会删除数据
