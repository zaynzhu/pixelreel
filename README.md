...
- **统一记录库** -- 电影、电视剧、游戏混排展示，支持分类/年份/状态多维筛选和评分短评；豆瓣短评展示会剔除页面附带的“有用”计数，但原始字段与快照保持不变；游戏详情将 Steam、Xbox、PSN 身份拆成独立平台档案，展示数据覆盖与最近同步，但不跨平台混算时长、成就或奖杯
...
- **后台任务恢复** -- 同步中心与命令抽屉共用持久化任务，服务重启后明确标记被中断任务，不再直接丢失状态；同步中心按当前临时账号计算待处理来源，并显示最近同步相对时间，超过 30 天才建议重新同步
- **临时账号记忆** -- Xbox Gamertag 与 PSN Online ID 可由用户选择保存在当前浏览器，取消勾选立即清除；密码和授权令牌不会进入浏览器存储
- **游戏平台遥测** -- 首页按平台档案汇总总游玩时长、档案数量和 Xbox/PSN 成就进度，并分别展示 Steam 游玩时长、Xbox 成就、PSN 奖杯、指标覆盖率与最近同步时间；记录库和游戏详情保留未知总数下可信的已解锁数量，并显示平台同步时间；详情额外展示可信完成率和首次导入时间；合并后的跨平台记录不会丢失来源指标
