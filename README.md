...
- **游戏平台遥测** -- 首页按平台档案汇总总游玩时长、档案数量和 Xbox/PSN 成就进度，并展示各平台指标覆盖率与最近同步时间；记录库和游戏详情按平台区分 Xbox 成就与 PSN 奖杯，保留未知总数下可信的已解锁数量，并显示平台同步时间；详情额外展示可信完成率和首次导入时间；合并后的跨平台记录不会丢失来源指标
