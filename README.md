...
- **导入审核队列** -- 新同步记录先进入待审核区，可按豆瓣、Trakt、Steam、Xbox、PSN 等稳定来源身份筛选并直接查看各来源数量、游戏成就进度和待核对重复组总数；已加载记录按“可直接处理、需核对、尚未检查”分流，可一键选择完成检查且未发现重复的记录，重复检查失败时禁止接受未经检查的游戏。未裁决的跨平台重复候选会在接受前提示，并可集中打开全部涉及待审核导入的候选或深链接定位到具体候选组。重复候选支持逐组核对、独立完成进度和前后切换，处理一组后继续显示下一组，当前范围清空后可直接返回导入审核；候选与合并预览会标明审核身份，并逐条列出保留、并入记录的平台与 ID。每组仍需单独预览和确认，返回时恢复原标签、来源和记录位置；接受或忽略采用全有或全无处理，队列并发变化时保留当前视图并提示刷新，忽略不会删除数据
- **首页行动入口** -- 首页直接显示电影、剧集和游戏的新同步记录待审核总数，并按同步中心同一口径提示需要处理的来源数量
- **资料库安全快照** -- 一键下载电影、剧集、游戏、游戏平台档案和豆瓣原始字段的只读 JSON，内含记录区 SHA-256 与分类计数；浏览器复算校验通过后才保存文件，不包含设置与密钥；上传 v2 快照先做只读恢复预览，二次确认后只补回快照独有数据，不覆盖、合并或删除现库记录
- **统一记录库** -- 电影、电视剧、游戏混排展示，支持分类/年份/状态多维筛选和评分短评；豆瓣短评展示会剔除页面附带的“有用”计数，但原始字段与快照保持不变；游戏详情将 Steam、Xbox、PSN 身份拆成独立平台档案，展示数据覆盖与最后同步时间，但不跨平台混算时长、成就或奖杯
- **时间线海报墙** -- 按月份分组的精美海报墙，支持年份切换和详情弹窗
- **雷达发现** -- 聚合 TMDB 热映/趋势 + 优酷/腾讯片单，一键加入想看列表
- **数据分析** -- 年度报告、月度趋势、评分分布、来源占比、游戏平台档案分布、跨平台评分对比；当前年度按截至当天的去年同期比较，历史年份按完整年度比较
- **数据健康审计** -- 检查缺失字段，识别疑似重复记录，可纠正来源匹配、确认不同，或在冲突校验后合并游戏记录；游戏合并可从操作日志撤销
- **大屏展示** -- 网格模式 + 全屏轮播模式，适合投屏展示你的影剧游收藏
- **操作日志** -- 自动记录每次 CRUD 操作，支持撤销和筛选
- **后台任务恢复** -- 同步中心与命令抽屉共用持久化任务，服务重启后明确标记被中断任务，不再直接丢失状态；同步中心按当前临时账号计算待处理来源，并显示同步完成时间的本地化相对表示，超过 30 天才建议重新同步
- **临时账号记忆** -- Xbox Gamertag 与 PSN Online ID 可由用户选择保存在当前浏览器，取消勾选立即清除；密码和授权令牌不会进入浏览器存储
- **游戏平台遥测** -- 首页按平台档案汇总总游玩时长、档案数量和 Xbox/PSN 成就进度，并分别展示 Steam 游玩时长、Xbox 成就、PSN 奖杯、指标覆盖率与最后同步时间；记录库和游戏详情保留未知总数下可信的已解锁数量，并显示平台同步时间；详情额外展示可信完成率和首次导入时间；合并后的跨平台记录不会丢失来源指标
- **安全合并预览** -- 跨平台重复游戏若恰好只有一条正式入库记录则保守建议将其作为主记录；合并前逐条展示平台公开 ID、原记录及保留或迁移去向，同时展示个人记录冲突，确认后仍保存完整快照并支持从操作日志撤销
...
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | 系统分层、数据模型和核心数据流 |
| [docs/PRODUCT_BEHAVIOR.md](docs/PRODUCT_BEHAVIOR.md) | 跨页面与跨接口的产品行为约束 |
| [docs/INTEGRATIONS.md](docs/INTEGRATIONS.md) | 豆瓣、Trakt、Steam、Xbox、PSN 接入 |
| [docs/RUNBOOK.md](docs/RUNBOOK.md) | 启动、配置、备份和故障排查 |
| [docs/HANDOFF.md](docs/HANDOFF.md) | 当前能力、接手顺序和后续方向 |
...
| `/sync/review` | 新导入审核队列（按来源筛选、查看游戏进度、重复候选往返定位、接受、忽略、修正） |
...
POST   /api/tools/restore-preview   校验快照并只读比较现库，不执行恢复
POST   /api/tools/restore           使用预览确认令牌执行增量恢复
...
| GamePlatformEntry | platform + externalId | 每个平台独立的游玩时长、成就/奖杯、首次导入和最后同步 |
